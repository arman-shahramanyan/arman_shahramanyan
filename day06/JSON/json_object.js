...
const dataArray = (data) => data.map( el => ({ name: el.name, position: el.hr.position, salary: el.hr.salary }));
console.log(dataArray(employeeDataParse));

// 2. Ստեղծել աշխատակիցների ցուցակի նոր զանգված որոնց աշխատավարձը >=150000;

const salaryFilter = data => data.filter(el => Number(el.hr.salary.replace(/[^0-9]/gi, "")) >= 150000);
console.log(salaryFilter(employeeDataParse));

// 3. Գտնել ամենից վաղ աշխատակցին; ( new Date() )

const difference = employeeDataParse.map( el => (Date.parse(new Date(el.hr.start_date)))).sort();
const firstEmployee = data => data.find(el => Date.parse(el.hr.start_date) == difference[0])
console.log(firstEmployee(employeeDataParse));
...
const employeeSalary = employeeDataParse.filter( el => Number(el.hr.salary.replace(/[^0-9]/g, '')) <= 100000);
const employeeBonus = employeeSalary.map(function (el) {
    el.hr.bonus = `$${ ( el.hr.salary.replace(/[^0-9]/g, '') * 20 / 100) }`;
    return el;
} );
...
const cities = employeeDataParse.map(el => el.contact[0]);
const uniqueCities = new Set(cities);
let employeeCount = {};
for (let i of uniqueCities) {
    let x = 0;
    for (let j of cities) {
        if(i == j) x++;
        // console.log(x)
...
const fireAnEmployee = data => {
    console.log("BOSS:Let me think");
    setTimeout( function () {
        const allEmployee = data.map(el => (el.name));
        const randomNum = Math.floor(Math.random() * 58);
        console.log(`All our employees: ${allEmployee}`);
        console.log(`Sorry friend but we have to say goodbye: ${allEmployee[randomNum]}`);
        delete allEmployee[randomNum];
        console.log(`New list of employees: ${allEmployee}`)
    }, 3000)        
}

fireAnEmployee(employeeDataParse);