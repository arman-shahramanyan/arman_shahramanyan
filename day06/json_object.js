...
const employeeBonus = (data) => data.filter((n) => Number(n.hr.salary.replace(/[^0-9]/g, '')) <= 100000)

const bonus = (data) => data.map( el => ({ ...n, hr: { ...el.hr, bonus: `$${Number(el.hr.salary.replace(/^0-9/g, '')) * 20 / 100}` } }));

console.log(bonus(employeeBonus(employeeDataParse)));
